module agda.proofs.host-sentry-copy where
...
prf refl (fst , snd) (step-NoOp p [[ pc , reg , true , UR , SR , ret-pc ]] prf₁ cmd-prf) = [ pc , reg ] , (step-NoOp ⟨ NoOp , 0 ∷ 0 ∷ 0 ∷ [] ⟩ p [ pc , reg ] prf₁ cmd-prf) , refl , refl
prf refl (fst , snd) (step-Add {n} {dest} {r1} {r2} p [[ pc , registers , true , UR , SR , ret-pc ]] prf₁ cmd-prf) = [ suc (pc) , registers  ] , ({! step-Add  !} , refl , {!   !})
prf a b (step-Sub _ _ prf₁ cmd-prf) = {!   !}
prf a b (step-Addi _ _ prf₁ cmd-prf) = {!   !}
prf a b (step-Jump _ _ prf₁ prf2 cmd-prf) = {!   !}
prf a b (step-Bgtz-l _ _ prf₁ prf2 prf3 cmd-prf) = {!   !}
prf a b (step-Bgtz-g _ _ prf₁ prf2 prf3 cmd-prf) = {!   !}
prf a b (step-Call-Unt _ _ prf₁ prf2 prf3 cmd-prf) = {!   !}
prf a b (step-Ret-Unt _ _ prf₁ prf2 prf3 cmd-prf) = {!   !}
prf a b (step-Return _ _ prf₁ cmd-prf) = {!   !}
prf a b (step-Alert _ _ prf₁ cmd-prf) = {!   !}
-- prf refl a (step-NoOp p ([[ pc , reg , mode , UR , SR , ret_pc ]]) prf₁ cmd-prf) = [ pc , reg ] , {! !} , refl , refl
...
-- prf refl refl refl (step-Ret-Unt p ([[ pc , reg , true , UR , SR , ret-pc ]]) prf₁ prf2 prf3 cmd-prf) = [ ret-pc , SR ] , {! step-Ret-Unt  !} , (refl , refl)
