...
-- trusted  
prf : ∀ {n temp} {p : Program n} {t : Trace} {sₕ : Hstate} {sₛ sₛ' : State}
    → (sₕ .Hstate.mode ≡ true)
    → (equiv sₕ sₛ)
    → t , p , sₛ —→ sₛ'
    → (sₕ .Hstate.UR ≡ temp)                            -- Trust that `temp` in trace matches `Host.UR`
...
prf refl (fst , snd) (step-NoOp _ _ _ prf-cur prf-cmd prf-trace prf-canStep) refl = {!   !}
prf refl (fst , snd) (step-Add t _ _ prf-cur prf-cmd prf-trace prf-canStep) d = {!   !}
prf refl (fst , snd) (step-Sub t _ _ prf-cur prf-cmd prf-trace prf-canStep) d = {!   !}
prf refl (fst , snd) (step-Addi t _ _ prf-cur prf-cmd prf-trace prf-canStep) d = {!   !}
prf refl (fst , snd) (step-Jump _ _ _ prf-cur prf-canStep prf-cmd prf-trace) d = {!   !}
prf refl (fst , snd) (step-Bgtz-l _ _ _ prf-cur prf-zero prf-cmd prf-trace prf-canStep) d = {!   !}
prf refl (fst , snd) (step-Bgtz-g _ _ _ prf-cur prf-canStep prf-gzero prf-cmd prf-trace) d = {!   !}
prf refl (fst , snd) (step-Call-Unt-Sentry _ _ _ prf-cur prf-canStep prf-cmd prf-canReturn prf-trace) d = {!   !}
prf refl (fst , snd) (step-Return _ _ _ prf-cur prf-cmd prf-trace) d = {!   !}
prf refl (fst , snd) (step-Alert _ _ _ prf-cur prf-cmd prf-trace) d = {!   !}
prf refl (fst , snd) (step-Load-UR _ _ _ prf-cur prf-cmd prf-canStep prf-trace) d = {!   !}
-- prf {n} {p} {⟨ NoOp , (0 ∷ 0 ∷ 0 ∷ []) ⟩} {h} {s} {s'} refl (refl , refl) 
--         (step-NoOp _ p [ pc , reg ] prf-cur prf-cmd prf-trace prf-canStep)   
--         = 
--         [[ pc , reg , true , _ , _ , _ ]] , 
--         step—→ p h
...
-- prf {n} {p} {t} {h} {s} {s'} refl (refl , refl) 
...
-- prf {n} {p} {⟨ Jump _ , _ ∷ 0 ∷ 0 ∷ [] ⟩} {h} {s} {s'} refl (refl , refl) 
...
-- prf {n} {p} {⟨ Bgtz _ _ , _ ∷ _ ∷ 0 ∷ [] ⟩} {h} {s} {s'} refl (refl , refl) 
...
-- prf {n} {p} {⟨ Return , 0 ∷ 0 ∷ 0 ∷ [] ⟩} {h} {s} {s'} refl (refl , refl) 
...
-- prf {n} {p} {⟨ Alert , 0 ∷ 0 ∷ 0 ∷ [] ⟩} {h} {s} {s'} refl (refl , refl) 
...
-- prf {n} {p} {⟨ Call-Unt-Sentry , 0 ∷ 0 ∷ 0 ∷ [] ⟩} {h} {s} {s'} refl (refl , refl) 
...
-- -- prf {n} {p} {t} {h} {s} {s'} refl (refl , refl) 
-- --         (step-Load-UR-Sentry _ p [ pc , reg ] prf-cur prf-cmd prf-canStep)  
-- --         = 
-- --         [[ suc pc , {!   !} , true , Hstate.UR h , Hstate.SR h , Hstate.ret-pc h ]] , 
-- --         step—→ p h 
-- --         [[ suc pc , State.registers s' , true , Hstate.UR h , Hstate.SR h , Hstate.ret-pc h ]] _
-- --         t _ 
-- --         (done p _ _) 
...
-- --         (refl , {!   !})  
-- -- prf {n} {p} {t} {h} {s} {s'} refl (refl , refl) 
-- --         (step-Load-UR _ p [ pc , reg ] prf-cur prf-cmd prf-canStep prf-trace)                            
-- --         = 
-- --         [[ (suc (pc)) , updateAt reg _ (λ x → 5) , true , Hstate.UR h , Hstate.SR h , Hstate.ret-pc h ]] , 
-- --         step—→ p h 
-- --         [[ suc pc , updateAt reg _ (λ x → 5) , true , _ , _ , _ ]] _
-- --         t _ 
-- --         (done p _ _) 
-- --         {! step-Load-UR  !} ,
...
-- prf {n} {p} {⟨ Load-UR _ , _ ∷ 0 ∷ 0 ∷ [] ⟩} {h} {s} {s'} refl (refl , refl) 
--         (step-Load-UR _ p [ pc , reg ] prf-cur prf-cmd prf-canStep prf-trace)                           
--         =
--         [[ (suc (pc)) , State.registers s' , true , Hstate.UR h , Hstate.SR h , Hstate.ret-pc h ]] , 
--         step—→ p h
--         [[ suc pc , State.registers s' , true , Hstate.UR h , Hstate.SR h , Hstate.ret-pc h ]] _        
--         ⟨ Load-UR _ , Hstate.UR h ∷ 0 ∷ 0 ∷ [] ⟩ _ 
--         (done p _ _) 
