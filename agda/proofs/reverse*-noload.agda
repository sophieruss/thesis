...
-- prf {n} {dest} {temp} {p} {⟨ Load-UR-Sentry _ zz , 0 ∷ 0 ∷ 0 ∷ [] ⟩} {h} {s} {s'} refl (refl , refl) zz 
--         (step-Load-UR {n} {a} {dest} (⟨ Load-UR-Sentry _ _ , 0 ∷ 0 ∷ 0 ∷ [] ⟩) p [ pc , reg ] prf-cur prf-cmd prf-canStep prf-trace) =
--         let
--             ur-val = Hstate.UR h
--             temp≡ur : temp ≡ ur-val
--             temp≡ur = temp≡ur
--             h' = [[ suc pc , updateAt reg dest (λ x → Hstate.UR h) , true , Hstate.UR h , Hstate.SR h , Hstate.ret-pc h ]]
--             regs-equal : updateAt reg dest (λ _ → ur-val) ≡ updateAt reg dest (λ _ → a)
--         in
--             h' , 
--             step—→ p h 
--                 h' _ 
...
-- prf {n} {dest} {temp} {p} {⟨ Load-UR-Sentry dest temp , 0 ∷ 0 ∷ 0 ∷ [] ⟩} {sₕ} {sₛ} {sₛ'} h-mode (refl , refl) temp≡ur (step-Load-UR .(⟨ Load-UR-Sentry dest temp , 0 ∷ 0 ∷ 0 ∷ [] ⟩) p .sₛ prf-cur prf-cmd prf-canStep refl) =
--   {!   !}
--   let
